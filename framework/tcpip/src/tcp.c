...
void TCPInit(void)
{
    ...
    for (i = 0; i < TCP_SOCKET_COUNT; i++) {
        ...
        default:
            while (1)
               ; // Undefined allocation medium.  Go fix your tcpip_config.h TCP memory allocations.
        }
 ...
 * Server sockets &#45; the local TCP port on which to
  ...
  Remarks:
    ...
    type, the DNS client module must also be enabled (STACK_USE_DNS_CLIENT must be
...
#if defined(STACK_CLIENT_MODE) && defined(STACK_USE_DNS_CLIENT)
...
#if defined(STACK_USE_SSL)
    if (MyTCBStub.sslStubID != SSL_INVALID_ID) { // Use sslTxHead as the head pointer when SSL is active
        uint16_t rem;
...
static bool FindMatchingSocket(TCP_HEADER* h, NODE_INFO* remote)
{
    ...
    for (hTCP = 0; hTCP < TCP_SOCKET_COUNT; hTCP++) {
        ...
        if (MyTCBStub.smState == TCP_CLOSED) {
            continue;
        } else if (MyTCBStub.smState == TCP_LISTEN) { // For listening ports, check if this is the correct port
            ...
            continue;
        } else if (MyTCBStub.remoteHash.Val != hash) { // Ignore if the hash doesn't match
            continue;
        }
...
static uint16_t GetMaxSegSizeOption(void)
{
    ...
    while (vOptionsBytes--) {
        ...
        if (vOption == 2u) // Maximum Segment Size option
        {
            ...
            // Get option length
            vOption = MACGet();
            if (vOption == 4u) { // Retrieve MSS and swap value to little endian
            ...
            if (wMSS > TCP_MAX_SEG_SIZE_TX)
                ...
                return wMSS;
        } else { // Assume this is a multi byte option and throw it way
            if (vOptionsBytes < 2u)
                break;
            vOption = MACGet();
...
static void HandleTCPSeg(TCP_HEADER* h, uint16_t len)
{
    ...
    case TCP_CLOSING:
        ...
        if ((int32_t) dwTemp > 0) { // acknowledged more than we've sent??
...
bool TCPStartSSLClient(TCP_SOCKET hTCP, uint8_t * host)
{
    ...
    for (i = 0; i < 5u; i++) { // Skip first 5 bytes in TX for the record header
        if (++MyTCBStub.sslTxHead >= MyTCBStub.bufferRxStart)
            MyTCBStub.sslTxHead = MyTCBStub.bufferTxStart;
    }
    return true;
...
void TCPSSLDecryptMAC(TCP_SOCKET hTCP, ARCFOUR_CTX* ctx, uint16_t len)
{
    ...
    while (len) {
        ...
        if (wSrc + wBlockLen > MyTCBStub.bufferEnd) { // Two part read
        ...
        if (wDest + wBlockLen > MyTCBStub.bufferEnd) { // Two part write
...
void TCPSSLInPlaceMACEncrypt(TCP_SOCKET hTCP, ARCFOUR_CTX* ctx, uint8_t * MACSecret, uint16_t len)
{
    ...
    for (blockLen = 0; blockLen < 5u; blockLen++) { // Skips first 5 bytes for the header
...
void TCPSSLPutRecordHeader(TCP_SOCKET hTCP, uint8_t * hdr, bool recDone)
{
    ...
    if (hdr) { // This is a new record, so insert the header
        for (i = 0; i < 5u; i++) {
    ...
    if (MyTCBStub.sslRxHead != MyTCBStub.rxHead) {
        ...
        do {
            ...
            if (wSSLBytesThatPoofed) {
                ...
                if (MyTCB.sHoleSize == -1) { // Just need to move pending SSL data
                    wToMove = TCPIsGetReady(hTCP);
                } else { // A TCP hole exists, so move all data
